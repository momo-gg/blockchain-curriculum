...
> — Common saying in the crypto community

---

## Why This Matters

Ethereum proved that programmable blockchains work. Perhaps too well. By 2021, the network was processing billions in daily transactions, but this success exposed fundamental limitations: fees spiked to $100+ for simple swaps, transactions took minutes to confirm, and the network consumed as much energy as a small country.
...
```
Block Space: 15 million gas (Ethereum's block limit)
...
> 💡 **Key Insight**: High fees aren't a bug — they're the market working. But they make Ethereum unusable for small transactions and many use cases.

### The Speed Perception Problem

Here's a question that confuses many people:

**"If Visa processes 65,000 transactions per second and is centralized, why can Solana (also fast) be decentralized while Ethereum can't?"**
...
|--------|------------|----------|--------|------|
| **Throughput** | Transactions per second | ~15 TPS | ~5,000 TPS | ~65,000 TPS |
| **Latency** | Time until included in block | ~12 seconds | ~400ms | ~2 seconds |
| **Finality** | Time until irreversible | ~15 minutes | ~12 seconds | ~60-90 days |

**Wait, Visa takes 60-90 days for finality?**

Yes! When you swipe your card:
1. **Authorization** (2 seconds): Visa approves the transaction
2. **Settlement** (24-48 hours): Money actually moves between banks
3. **Chargeback window** (60-90 days): Transaction can still be reversed

Visa is "fast" for authorization but incredibly slow for true finality. Blockchains are slower for latency but much faster for finality.

> 💡 **Key Insight**: Blockchains trade authorization speed for finality speed. Once a blockchain transaction is final, it cannot be reversed by anyone, ever. This is a fundamentally different guarantee than Visa provides.
...
It's not "cheating" — it's choosing different points on the trilemma.
...
Attack cost: Buy 33-51% of all staked ETH + lose it
```
...
> ⚠️ **Common Misconception**: "Proof of Stake is less secure than Proof of Work." Neither is inherently more secure — they have different security models and trade-offs.
...
- ✅ Decentralization: Low hardware requirements, many nodes
- ✅ Security: High economic security, battle-tested
- ❌ Scalability: 15 TPS, high fees during congestion
...
│ Tx 1: A→B   │
│ Tx 2: C→D   │    Compressed
│ Tx 3: E→F   │  ─────────────→   Single L1 tx with proof
│    ...      │
│ Tx 1000     │
└─────────────┘
...
> 📊 **Business Context**: Layer 2s reduce fees 10-100x but fragment liquidity and add complexity. Users must bridge assets, and composability across L2s is limited. It's a trade-off, not a free lunch.

### The Merge: Ethereum's Transition to PoS

On September 15, 2022, Ethereum switched from PoW to PoS — the largest live software migration in crypto history.
...
> 📊 **Business Context**: The Merge was a governance success — coordinating thousands of node operators, billions in value, and years of development without central authority. It demonstrates that decentralized systems can make major changes.
...
> 💡 **Key Insight**: PoS security is economic, not cryptographic. Attacking is possible but economically irrational. This is similar to how burglary is possible but prison risk deters most people.
...
Ethereum's PoS uses "Gasper" — combining Casper FFG (finality) and LMD-GHOST (fork choice):
...
│   └── Committee of validators attest
├── Slot 1
│   ├── Different proposer
│   └── Different committee
...
└── Slot 31
    └── Epoch boundary (finality checkpoint)
```
...
Validators agreeing on checkpoint: 700,000 ETH (70%) ✅
...
```
Penalty = (individual_penalty) × (validators_slashed / total_validators) × 3
...
- Block >50% full → base_fee increases
- Block <50% full → base_fee decreases
```

**Why Burning?**

```
Before: More activity → more fees to validators
After: More activity → more ETH burned → reduced supply
...
1. Validator: Buy Token X ← Frontrun
2. Large buy order (price goes up)
3. Validator: Sell Token X ← Profit
4. Bob: Buy Token X (worse price)
```

Priority fees are essentially bribes for ordering. This leads to Flashbots, MEV auctions, and eventually — why Solana does things differently.
...
→ [2.3 Stablecoins: Digital Dollars](./2.3-stablecoins.md)
