...
> 💡 **Key Insight**: You're trusting Circle to actually hold the reserves and honor redemptions. This is counterparty risk — the exact thing Bitcoin was designed to avoid.
...
- Coinbase receives 50% of USDC reserve interest income
- This revenue split is worth hundreds of millions annually
...
> 💡 **Key Insight**: Stablecoin issuers make money on your float. You get convenience; they get interest-free deposits.
...
- Not available to US users (regulatory shield)
- "Too big to fail" psychology

> 📊 **Business Context**: USDT's dominance despite trust issues shows network effects in money. People use what others use, even if alternatives are "better." This is why USDC struggles to gain market share.
...
**Native USDC**
- Circle directly issues on: Ethereum, Solana, Polygon, Arbitrum, etc.
- Circle can freeze/mint/burn directly
...
Circle issues → User holds real USDC

Bridged USDC (e.g., via Wormhole):
Circle issues on Ethereum → Bridge locks it → Bridge mints "USDC" on Chain X
                                              ↑
...
> 💡 **Key Insight**: Private stablecoins are innovating faster than CBDCs because they don't need government coordination. But CBDCs may eventually regulate or replace them.
...
> 📊 **Business Context**: UST showed that "algorithmic" stablecoins are fundamentally fragile. The search for decentralized, scalable stablecoins continues, but every design has trade-offs.
...
Original USDC locked on Ethereum:
    └── Wormhole guardians attest lock
        └── Wrapped USDC minted on destination chain
...
```
Traditional Bridge:
  Ethereum USDC → Lock → Bridge IOU → Redeem → Target Chain

CCTP:
  Ethereum USDC → Burn → Circle attestation → Mint on Target Chain
...
If DAI > $1:
  ...
  - More DAI supply → price decreases

If DAI < $1:
  - Cheaper to repay DAI loans
  - Less DAI supply → price increases
...
1. **Stablecoins bridge TradFi and crypto** — essential for most DeFi activity
...
5. **Algorithmic stablecoins failed** — UST/Luna showed reflexive designs are fragile
6. **CBDCs are coming** but may complement rather than replace private stablecoins
7. **Circle can freeze your USDC** — decentralization is sacrificed for stability

Stablecoins are infrastructure. Now that we understand the programmable blockchain era — smart contracts, PoS, and stable units of account — we're ready to see how Solana does things differently.
