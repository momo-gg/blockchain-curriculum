...
Transaction arrives
    │
    ▼ (Gulf Stream: skip the waiting room)
Leader receives
    │
    ▼ (PoH: stamp with verifiable time)
Transaction ordered
    │
    ▼ (Sealevel: process in parallel)
Transaction executed
    │
    ▼ (Pipelining: like a CPU pipeline)
Block created
    │
    ▼ (Turbine: spread like gossip)
Block propagated
    │
    ▼ (Tower BFT: validators vote)
Block confirmed
    │
    ▼ (Cloudbreak: save to disk fast)
State updated
```
...
Traditional (Client-Server):
  All computers → Central Server → All computers
...
───────────────────          ────────────────────────────
Solana Protocol      →       Agave (Rust, by Anza)
                             Firedancer (C, by Jump)
                             Sig (Zig, by Syndica)

Just like:
HTTP Protocol        →       Chrome, Firefox, Safari
```
...
Solana Labs archived the original `solana-labs/solana` repo after Anza took over core validator development. The active validator client lives in the **Agave** repo, which is a maintained fork of the original Solana validator. The archive keeps history, but signals that mainline development moved.
...
```
Today: Agave dominates mainnet; Firedancer is in staged testing/deployments
...
Solana expects **one leader per slot**. Blocks from any other validator for that slot are rejected. There can be temporary forks during partitions, but each slot still has a single expected leader according to the leader schedule.

### Alpenglow (Proposed Consensus Rewrite)

**Alpenglow** is a proposed new consensus protocol by Anza that replaces TowerBFT and Proof of History with new components (Votor for voting/finality and Rotor for data dissemination). It is **not deployed** as of today; published targets are based on simulations and research. Treat performance numbers as aspirational until implemented and activated.
...
> 📊 **Business Context**: Feature gates enable upgrades without hard forks. This is more flexible than Ethereum but requires validator coordination. Controversial features still require social consensus.
...
Process:
1. Draft → 2. Review → 3. Last Call → 4. Accepted/Rejected
```
...
```
User → RPC → Mempool (waiting room) → Leader picks transactions
                    ↑
...
```
User → RPC → Forward directly to current/next leader
                    ↑
...
6. **Multiple clients improve resilience** — Agave + Firedancer in production
