...
User clicks "Swap"
  ...
  → RPC forwards to current leader
  → Leader executes in Sealevel
  → Block propagates via Turbine
  → Validators vote (Tower BFT)
...
───────────────────          ────────────────────────────
Solana Protocol      →       Agave (Rust, by Anza) (as of [docs.anza.xyz](https://docs.anza.xyz/))
...
Original Solana Labs client
    │
    ├── Agave (Anza) — mainnet client (as of [docs.anza.xyz](https://docs.anza.xyz/))
    ├── Firedancer (Jump Crypto) — new client in C (as of [jumpcrypto.com/firedancer](https://jumpcrypto.com/firedancer))
    └── Sig (Syndica) — Zig implementation (as of [github.com/syndica/sig](https://github.com/syndica/sig))
```
