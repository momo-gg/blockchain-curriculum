...
```
┌─────────────────────┐
│   Token Program     │     ← Just code, no data
│   (Deployed once)   │
└─────────────────────┘
          │
          │ operates on
          ▼
┌─────────────────────┐  ┌─────────────────────┐  ┌─────────────────────┐
│  Alice's Token      │  │  Bob's Token        │  │  Carol's Token      │
│  Account            │  │  Account            │  │  Account            │
│  ───────────────    │  │  ───────────────    │  │  ───────────────    │
│  Owner: Token Prog  │  │  Owner: Token Prog  │  │  Owner: Token Prog  │
│  Balance: 100       │  │  Balance: 50        │  │  Balance: 200       │
└─────────────────────┘  └─────────────────────┘  └─────────────────────┘
...
│  owner: TokenkegQ...           (Which program controls it) │
│  executable: false             (Is this code or data?)     │
│  rent_epoch: 432               (When rent was last paid)   │
│  data: [0x01, 0x00, ...]       (Arbitrary bytes)          │
└────────────────────────────────────────────────────────────┘
...
```
┌─────────────────────┐
│   System Program    │  ← Owns your wallet (native SOL accounts)
└─────────────────────┘
...
Storage isn't free. Accounts must pay "rent" to exist:

**The Rent Model**

```
Option 1: Pay rent continuously
  - Account loses lamports over time
  - If balance hits zero, account is purged

Option 2: Rent-exempt (what everyone uses)
...
```
Minimum balance = data_size × rent_per_byte × 2_years_of_rent

Example (token account, 165 bytes):
  165 bytes × 0.00000348 SOL/byte × 2 = ~0.00203 SOL
...
```
sha256(seeds + program_id + bump) → candidate address

Try bump = 255:
  If candidate is on the curve → not valid, try bump = 254
  If candidate is off the curve → found PDA!
...
Your Transaction
  └── Your Program (depth 0)
        └── Token Program (depth 1)
              └── Another Program (depth 2)
                    └── Yet Another (depth 3)
                          └── ... (max depth 4)
```
...
4. **Rent-exemption** — deposit ~2 years of rent for permanent storage
...
→ [3.4 Validators & Network Architecture](./3.4-validators-network.md)
