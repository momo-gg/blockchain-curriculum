...
> *"Validators are the backbone of the network. They're not just running software — they're running a business."*
...
> ⚠️ **Data Disclaimer**: Validator counts, stake distributions, and hardware costs cited here reflect late 2024 conditions. The validator set changes frequently — check [Solana Beach Validators](https://solanabeach.io/validators) or [Validators.app](https://www.validators.app/) for current data.
...
│         ↓                  ↓                  ↓                 │
│   ┌─────────────────────────────────────────────────────┐      │
│   │              Accounts Database (Cloudbreak)          │      │
│   └─────────────────────────────────────────────────────┘      │
│                                                                  │
│   CPU: 16+ cores  |  RAM: 256+ GB  |  Storage: 1+ TB NVMe     │
│   Network: 1+ Gbps                                              │
└─────────────────────────────────────────────────────────────────┘
```
...
More stake = More reputation and future rewards at risk (formal slashing is limited today)
...
> 💡 **Key Insight**: Node count isn't everything. Stake distribution matters more. Ethereum has more nodes but more concentrated stake (Lido controls ~32%).
...
Revenue per 1M staked SOL: 1M × 6% × 8% = 4,800 SOL
...
```
SIMD Lifecycle:
  Draft → Review → Last Call → Accepted/Rejected → Implemented → Activated
...
```
Single client risk:
  Bug in code → Network halts
  Bug affects everyone

Multiple clients:
  Bug in Agave → Firedancer validators keep running
...
**Schedule Properties**
- Same inputs → same schedule (any node can compute)
...
Epoch N: You stake 1000 SOL
  → Stake is "activating"
  → Not counted for leader schedule
  → Not earning rewards

Epoch N+1: Stake is "active"
  → Counted for leader schedule
  → Earning rewards
...
Epoch M: You unstake
  ...
  → Still earning (but declining proportionally)
...
```
Standard Validator:
  Transaction → Leader → Block
  Revenue: 50% of base fee

Jito Validator:
  Transaction → Jito Bundles → Searcher tips → Leader → Block
...
If validator charges 10% commission:
  Validator keeps: 0.95 SOL × 10% = 0.095 SOL
  Stakers receive: 0.95 SOL × 90% = 0.855 SOL
```
...
1. **Validators are businesses** — significant capital and operating costs
2. **Leaders are chosen by stake** — more stake = more slots = more responsibility
3. **Performance varies by leader** — but bad leaders lose stakers
4. **Multiple clients improve resilience** — Agave and Firedancer
5. **Governance is informal but functional** — SIMDs, stake-weighted activation
6. **Staking has warmup/cooldown** — prevents gaming
7. **Jito is standard** — ~90% of validators run it for MEV revenue

---

## Next

Now let's understand staking from the staker's perspective — how rewards work, how to choose validators, and the risks involved.

→ [3.5 Staking Mechanics](./3.5-staking-mechanics.md)
