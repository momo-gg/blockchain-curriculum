...
| Mempool | Public, searchable | No mempool (Gulf Stream) |
| MEV auction | Flashbots MEV-boost | Jito |
| Searcher speed requirement | Medium | Very high |
| MEV distribution | To validators | Validators + stakers (Jito) |
...
Solana:
  - Transactions go directly to leader
...
```
~90% of Solana validators run Jito client

Why validators run Jito:
  - Additional revenue from tips
  - Shared with their stakers
...
With Jito:
  ...
  - Validators get: Tips (shared with stakers)
...
6. **MEV is distributed** via Jito — validators and stakers get a cut
