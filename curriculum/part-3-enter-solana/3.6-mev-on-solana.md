...
> *"MEV is like someone peeking at your cards in a poker game and betting accordingly."*
...
```
   ...
   │            Sandwich Attack              │
   │                                         │
   │  1. Frontrun: Attacker buys             │
   │         ↓                               │
   │  2. Victim's transaction executes       │
   │         ↓                               │
   │  3. Backrun: Attacker sells             │
   │                                         │
   │  Victim is "sandwiched" between         │
   │  attacker's buy and sell                │
   └─────────────────────────────────────────┘
```
...
Attacker monitors for undercollateralized positions
  → Liquidates them first (earns liquidation bonus)
  → Legitimate liquidators lose opportunity
```
...
> 💡 **Key Insight**: MEV is like a tax on uninformed trading. It exists because of how markets work, not because of blockchain design. Blockchains just make it more visible.
...
| Mempool | Public, searchable | No public mempool (leader forwarding) |
| MEV auction | Flashbots MEV-boost | Jito |
| Searcher speed requirement | Medium | Very high |
| MEV distribution | To validators | Validators (some share with stakers) |

**Why Solana MEV Is Different**

```
Ethereum:
  - Transactions wait in public mempool
  - Anyone can see pending transactions
  - 12 seconds to prepare attack
...
│                      ▼                                       │
│              JITO BLOCK ENGINE                               │
│              (Auction for ordering)                          │
│                      │                                       │
│                      ▼                                       │
│              TIPS DISTRIBUTED                                │
│              (Validators; some share with stakers)           │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
Example Bundle (Sandwich):
  tx1: Buy Token X      ← Searcher's frontrun
  tx2: Victim's swap    ← Original user transaction
  tx3: Sell Token X     ← Searcher's backrun
  tip: 0.01 SOL         ← Payment to validator
...
```
Many Solana validators run a Jito client

Why validators run Jito:
  - Additional revenue from tips
  - Sometimes shared with their stakers (policy varies)
  - Competitive disadvantage not to

Why this is concerning:
  - Single infrastructure point
  - MEV becomes "legitimized"
  - Harder to avoid for users
```
...
```
Normal swap:
  You → Public RPC → Leader → Block

Jupiter protected swap:
  You → Jupiter → Jito (as bundle) → Leader → Block

Benefits:
  - Your transaction bundled atomically
  - Harder to sandwich
  - You might get rebate from MEV captured
```
...
```
Pre-Jito:
  - Searchers get: 100% of extracted value
  - Validators get: Maybe higher priority fees
  - Users get: Worse prices

With Jito:
  - Searchers get: Value minus tips
  - Validators get: Tips (may be shared with stakers)
  - Users get: Still worse prices, but some flows back

Example:
  MEV opportunity: 1 SOL
  Searcher profit after tip: 0.8 SOL
  Tip to validator: 0.2 SOL
  Validator keeps (10% commission): 0.02 SOL
  Stakers receive: 0.18 SOL
```
...
> - [ ] What does Jito do?
> - [ ] What percentage of validators run Jito?
...
├─────────────────────────────────────────────────────────────┤
│                                                              │
│   DATA INGESTION              OPPORTUNITY DETECTION          │
│   ┌─────────────┐             ┌──────────────────┐          │
│   │ Geyser      │─────────────│ Price Arbitrage  │          │
│   │ (Account    │             │ Detector         │          │
│   │  changes)   │             └──────────────────┘          │
│   └─────────────┘             ┌──────────────────┐          │
│   ┌─────────────┐             │ Sandwich         │          │
...
1. **MEV = value from transaction ordering** — affects everyone who trades
...
4. **Jito dominates Solana MEV** — ~90% of validators
5. **Protection options exist** — limit orders, private RPCs, MEV protection
6. **MEV is distributed** via Jito — validators receive tips, and some share with stakers
7. **No perfect solution** — MEV is fundamental to open markets
...
→ [3.7 Solana vs. The Competition](./3.7-solana-vs-competition.md)
