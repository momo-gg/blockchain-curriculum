...
> ⚠️ **Note**: Resource limits and compute costs vary by release. Verify current values in the Solana source code (as of [`solana-labs/solana`](https://github.com/solana-labs/solana)).
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    Solana Resource Limits                           │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│   COMPUTE UNITS                                                     │
│   Default: cluster-configured                                       │
│   Maximum: cluster-configured                                       │
│   Per instruction: varies by operation                              │
│                                                                      │
│   TRANSACTION SIZE                                                  │
│   Maximum: version-dependent                                        │
│   Includes: signatures, addresses, instructions, data               │
│                                                                      │
│   ACCOUNT DATA                                                      │
│   Maximum per account: protocol-defined                             │
│   Maximum realloc per instruction: protocol-defined                 │
│                                                                      │
│   ACCOUNTS PER TRANSACTION                                          │
│   Legacy messages: protocol-defined                                 │
│   v0 messages: more via address lookup tables                       │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
1. ALGORITHM FIRST
   ...
   - O(n) vs O(n²) matters more than anything
...
```
OPTIMIZE WHEN:
  ✓ Transaction fails due to CU limit
  ✓ Complex operations timing out
  ✓ Users complaining about costs
  ✓ Approaching account size limits

DON'T OPTIMIZE WHEN:
  ✗ Code is still in development
  ✗ No performance problems exist
  ✗ Optimization hurts readability significantly
  ✗ Before you've measured the actual problem
```
...
pub fn best(ctx: Context<MyContext>) -> Result<()> {
    ...
    // find_program_address costs ~1500 CU
    // create_program_address with known bump costs ~200 CU
    Ok(())
}
```
...
pub fn multiple_transfers(ctx: Context<MultiTransfer>) -> Result<()> {
    ...
    token::transfer(ctx1, 100)?;  // ~4000 CU
    token::transfer(ctx2, 200)?;  // ~4000 CU
    token::transfer(ctx3, 300)?;  // ~4000 CU
    // Total: ~12000 CU
...
```
□ Stored bumps instead of finding PDAs
□ Minimized CPI calls
□ Right-sized data types
□ Used bit flags for booleans
□ Zero-copy for large accounts
□ Address lookup tables for many accounts
□ Hoisted loop invariants
□ Early exits in loops
□ Measured before optimizing
□ Profiled compute unit usage
```
...
2. **Store bumps** — avoid find_program_address
3. **Minimize CPIs** — each has ~4000 CU overhead
4. **Right-size types** — u8 vs u64 matters
5. **Zero-copy for large data** — skip deserialization
6. **Use lookup tables** — compress transaction size
7. **Request more CUs** — when needed with priority fee
8. **Algorithm over micro-optimization** — big-O wins
...
→ [Part 5: Client Development](../part-5-client-development/5.1-client-architectures.md)
