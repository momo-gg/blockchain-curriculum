...
In this module, you'll build a complete Solana program from scratch — a simple counter that can be incremented and read. This covers the fundamental patterns you'll use in every Solana program.
...
- **Stateless**: The program itself has no memory — it's just rules
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│   PROGRAM (The Rules)              ACCOUNTS (The Data)              │
│   ┌─────────────────┐              ┌─────────────────┐              │
│   │ Counter Program │              │ Counter Account │              │
│   │                 │              │ ┌─────────────┐ │              │
│   │ initialize()    │◄────────────►│ │ count: 42   │ │              │
│   │ increment()     │   reads/     │ │ authority:  │ │              │
│   │ reset()         │   writes     │ │  Alice      │ │              │
│   │                 │              │ └─────────────┘ │              │
│   └─────────────────┘              └─────────────────┘              │
│                                                                      │
│   The program defines              Accounts store the               │
│   WHAT can happen                  actual values                    │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
- The **program** is the rulebook — it never changes during the game
- The **accounts** are the game pieces and score cards — they change as you play
- **Instructions** are like taking a turn — "I want to move my piece 3 spaces"
...
User calls increment
  ...
  → Missing signer? Transaction fails
  → No state change occurs
```

Even small missing checks can break user flows, which is why tests must cover
...
│                                                                      │
│   1. Write Rust code                                                 │
│   2. anchor build (compiles, generates IDL + types)                  │
│   3. anchor test (runs local validator, executes tests)              │
│   4. Fix issues, repeat 1-3                                          │
│   5. anchor deploy (push to devnet/mainnet)                          │
│                                                                      │
│   Typical iteration: 30 seconds to rebuild and test                  │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
- `use anchor_lang::prelude::*` — imports common Anchor types
- `declare_id!` — your program's unique address on-chain
...
- `#[program]` — marks this module as containing instruction handlers
- Each `pub fn` is an instruction clients can call
- `Context<T>` — contains accounts for instruction `T`
...
- `#[account]` — marks as an Anchor account type
- `#[derive(InitSpace)]` — auto-calculates space needed
...
#   ✔ Resets the counter
#   ✔ Fails when non-authority tries to reset
#
# 5 passing
```
...
Client (TypeScript)
    ...
    ▼ Send to Solana
        │
        ▼ RPC receives, forwards to leader
            │
            ▼ Leader executes program
                │
                ▼ Anchor validates accounts:
                ...
                ▼ Your code runs:
                    ...
                    - counter.bump = bump
                │
                ▼ Anchor serializes counter account data
                │
                ▼ Transaction committed to block
    │
...
Account on-chain:
┌─────────────────────────────────────────────────────────────────┐
│ Address: 7xKX...abc123 (the PDA)                                │
├─────────────────────────────────────────────────────────────────┤
│ Owner: CounterProgram111...                                     │
│ Lamports: 1141440 (rent-exempt minimum)                         │
│ Data: [discriminator][authority][count][bump]                   │
...
2. **PDAs provide deterministic addresses** — derive from seeds
3. **Anchor validates accounts** — constraints catch errors early
4. **Every instruction needs accounts** — specified in the `Accounts` struct
5. **Tests should cover happy path AND errors** — verify authorization works

---

## Next

Let's dive deeper into accounts and PDAs — the foundation of Solana programs.

→ [4.3 Deep Dive: Accounts & PDAs](./4.3-accounts-and-pdas.md)
