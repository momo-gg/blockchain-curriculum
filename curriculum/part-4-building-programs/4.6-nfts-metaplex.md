...
│   (SPL Token)                 (Metaplex)                            │
│   ┌─────────────┐             ┌─────────────────────┐               │
│   │ Supply: 1   │◄───────────►│ Name: "Example NFT" │               │
│   │ Decimals: 0 │             │ Symbol: "CAPE"      │               │
│   │ Authority:  │             │ URI: "https://..."  │               │
│   │   (frozen)  │             │ Creators: [...]     │               │
│   └─────────────┘             │ Collection: Pubkey  │               │
│         │                     │ Uses: (optional)    │               │
│         │                     └─────────────────────┘               │
│         ▼                                                           │
│   TOKEN ACCOUNT               MASTER EDITION                        │
│   (Who owns it)               (Proof of uniqueness)                 │
│   ┌─────────────┐             ┌─────────────────────┐               │
│   │ Owner: Alice│             │ MaxSupply: 0        │               │
│   │ Amount: 1   │             │ (No prints allowed) │               │
│   └─────────────┘             └─────────────────────┘               │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
Owning an NFT usually means:
- The token itself (proof of ownership)
- The right to transfer or sell it
- The right to display it under the project's license

It usually does **not** grant intellectual property rights unless the project
explicitly says so. Always check the collection's license terms.
...
VERIFIED COLLECTION:
  ┌─────────────────────┐
  │ Collection NFT      │ ← Master NFT representing the collection
  │ Name: "Cool Apes"   │
  │ Verified: true      │
  └─────────────────────┘
          │
  ...
  ┌─────────────────────┐
  │ Item NFT #42        │
  │ Collection: ✓       │ ← Verified as belonging to collection
  │ Parent: Cool Apes   │
  └─────────────────────┘
...
Secondary Sale:
  ...
  Buyer pays SOL
  ├── Creator receives royalty (if enforced by marketplace)
...
Programmable NFTs can enforce royalties at the protocol level by applying
transfer rules. This makes it harder for marketplaces to bypass creator fees,
but support varies by marketplace and wallet.

### Marketplace Flow (User View)

```
List NFT → Sign listing → NFT stays in wallet → Buyer purchases → Transfer settles
```

Listings are usually "offers to sell" rather than custody transfers. The NFT
remains in the wallet until the sale executes.

### NFT Security Checklist

- Verify official collection links and marketplaces
- Review transaction details before approving
- Avoid signing "blanket approvals" for unknown programs
- Use a separate wallet for high-value NFTs
...
> ⚠️ **Note**: The older `@metaplex-foundation/js` SDK is deprecated. Use `@metaplex-foundation/umi` for new projects.
...
2. **Metaplex adds metadata** — name, image, attributes
3. **Master Edition** proves uniqueness and controls prints
4. **Collection verification** prevents counterfeits
5. **Royalties are optional** — increasingly opt-in on marketplaces
6. **Compressed NFTs** are 99% cheaper for large collections
7. **Same token operations** — transfer, burn work identically

---

## Next

Learn about Token Extensions (Token-2022) — the next generation of tokens on Solana.

→ [4.7 Token Extensions (Token-2022)](./4.7-token-extensions.md)
