...
Solana programs handle real money. A bug in production can mean permanent loss of funds. Unlike traditional software where you can quickly patch issues, deployed programs are immutable (unless using upgradeable patterns). Comprehensive testing is not optional — it's essential.
...
│                   └─────────────────────┘                           │
│              ┌───────────────────────────────┐                      │
│              │   Program Tests               │   BankClient          │
│              │   (Anchor)                    │   Fast iteration      │
│              └───────────────────────────────┘                      │
│         ┌─────────────────────────────────────────┐                 │
...
| **Reentrancy** | Can't exploit CPIs | Critical |
...
✓ Every constraint validation
✓ Every error path
✓ Account ownership checks
✓ Signer requirements
✓ PDA derivations
✓ Math operations (especially divisions)
✓ State transitions
```
...
1. **Test everything** — bugs mean lost funds
2. **Multiple layers** — unit, integration, E2E
3. **Error paths** — test what should fail
4. **Authorization** — verify access controls
5. **Edge cases** — zero, max, overflow
6. **State transitions** — before/after verification
7. **Fuzz testing** — random inputs for robustness
8. **Local first** — iterate fast, deploy when ready
...
→ [4.9 Security & Common Vulnerabilities](./4.9-security-vulnerabilities.md)
