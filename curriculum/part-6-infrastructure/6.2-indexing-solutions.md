# 6.2 Indexing Solutions

> *"The blockchain stores everything — but finding it is another story."*
...
│   ┌──────────────────┐                ┌──────────────────┐          │
│   │ "Find all NFTs   │                │ "Find all NFTs   │          │
│   │  owned by user"  │                │  owned by user"  │          │
│   └────────┬─────────┘                └────────┬─────────┘          │
│            │                                   │                     │
│            ▼                                   ▼                     │
│   Scan ALL accounts                   Query indexed database        │
│   (millions of accounts)              (milliseconds)                │
│            │                                   │                     │
│            ▼                                   ▼                     │
│   ⏱️ Minutes to hours                 ⏱️ < 100ms                    │
│   💰 Expensive compute                💰 Simple lookup               │
│   ❌ Impractical                      ✅ Production-ready            │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
```
RPC/Geyser → Ingest → Transform → Store → Query API
```
...
│   ├── NFT galleries, marketplaces  ├── Helius DAS API               │
│   ├── Token analytics              ├── Helius Enhanced APIs         │
│   ├── Custom complex queries       ├── The Graph subgraphs          │
│   ├── Real-time streaming          ├── Geyser plugins               │
│   ├── Historical transaction data  ├── Helius/Shyft APIs            │
│   └── Full control requirements    └── Custom indexer               │
│                                                                      │
│   TEAM CONSIDERATIONS                                                │
│   ├── < 3 devs: Use managed solutions (Helius, Shyft)               │
│   ├── 3-10 devs: Consider The Graph or managed + custom             │
│   └── > 10 devs: Custom indexer may be justified                    │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
1. **Don't query RPC for historical data** — use indexers
2. **Managed APIs first** — Helius, Shyft handle most use cases
3. **DAS API for NFTs** — standard for NFT/token queries
4. **Geyser for real-time** — lowest latency streaming
5. **Custom indexers for specific needs** — complex queries, full control
6. **Cache aggressively** — reduce indexer load
7. **Consider maintenance burden** — managed vs self-hosted
8. **Plan for scale** — indexer choice affects scaling options

---

## Next

Learn about deploying Solana programs to different environments.

→ [6.3 Deployment Strategies](./6.3-deployment-strategies.md)
