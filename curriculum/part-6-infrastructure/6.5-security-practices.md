...
> *"In blockchain, security is not a feature — it's the foundation."*
...
```typescript
// ❌ NEVER: Store keys in code or environment variables for production
...
```typescript
// ✅ Verify transactions match user intent
...
// ✅ Phishing warning for wallet connections
function WalletConnectWarning() {
  return (
    <div className="bg-yellow-100 p-4 rounded-lg mb-4">
      <h3 className="font-bold">Security Reminder</h3>
      <ul className="text-sm mt-2">
        <li>✓ Verify you're on {window.location.hostname}</li>
        <li>✓ Check the URL starts with https://</li>
...
// ✅ Timelock pattern for sensitive operations
#[account]
...
1. **Never store keys in code** — use HSMs or secret managers
2. **Multisig for critical operations** — remove single points of failure
3. **Audit before mainnet** — invest in professional security review
4. **Simulate all transactions** — catch errors before on-chain
5. **Rate limit everything** — protect against abuse
6. **Validate all inputs** — trust nothing
7. **Monitor continuously** — detect incidents early
8. **Have incident plans** — know what to do when things go wrong

---

## Next

Learn about production considerations for Solana applications.

→ [6.6 Production Considerations](./6.6-production-considerations.md)
