---
title: "1.2 Bitcoin: The First Solution"
description: "How Bitcoin solved the trust problem through cryptography, proof-of-work, and decentralized consensus."
---
...
On Halloween 2008, a pseudonymous person (or group) named Satoshi Nakamoto posted a 9-page paper to a cryptography mailing list. Two months later, the network went live. This wasn't just new software — it was the first working solution to a problem computer scientists had debated for decades.

Understanding how Bitcoin works is essential because every blockchain since — including Solana — is either building on Bitcoin's ideas or explicitly departing from them.
...
> If making it expensive to lie, and profitable to tell the truth, no central authority is needed — rational actors will maintain the system honestly because it's in their financial interest.
...
```
      ...
      │           MINING COMPETITION          │
      │                                       │
      │  Miner A: ❌ Wrong answer            │
      │  Miner B: ❌ Wrong answer            │
      │  Miner C: ✅ CORRECT! Adds block     │
      │  Miner D: ❌ Wrong answer            │
      │                                       │
      │  Miner C earns: 6.25 BTC reward      │
      │  Everyone else: Verify C's work      │
      └───────────────────────────────────────┘
```

> 💡 **Key Insight**: The "puzzle" isn't useful work — it's deliberately wasteful. The waste is the point. It makes cheating expensive because you'd have to spend real money (electricity) to lie.
...
```
Honest Network Hashpower: ████████████████████ 95%
Alice's Hashpower:        █ 5%

Alice cannot outrun the honest chain.
```

> ⚠️ **Common Misconception**: "Bitcoin uses a lot of energy to process transactions." Not exactly. Bitcoin uses energy to secure the network against attacks. The energy is a security budget.
...
Bitcoin proved:
- ✅ Strangers who don't trust each other can maintain a shared truth
- ✅ Digital scarcity is possible
- ✅ Economic incentives can replace institutional trust
...
Bitcoin isn't just a technical system — it's an economic one. Satoshi designed incentives carefully:
...
> 📊 **Business Context**: Bitcoin's predictable monetary policy contrasts with central banks, which can create unlimited currency. This appeals to those who distrust monetary policy decisions.
...
```
Profitable when: (BTC earned × BTC price) > Operating costs
...
> 💡 **Key Insight**: This is an unsolved problem. Ethereum and Solana take different approaches to this challenge.

### Network Effects and Lindy Effect

Bitcoin's primary moat isn't technology — it's network effects:

**Network Effect Types**
| Type | Description | Bitcoin's Advantage |
|------|-------------|---------------------|
| Direct | More users → more useful | More holders → more liquidity → easier to buy/sell |
| Indirect | More users → more infrastructure | More exchanges, wallets, ATMs, merchants |
| Data | More users → more data → better product | More miners → more security → more trust |
...
Signature: "4d2c1a..."
     │
     ▼ (Anyone can verify)
Is Alice's public key valid for this signature? ✅
```
...
>
> **Why this is clever**: If someone claims "Team A played in this tournament," you don't need to check every game. You only need to trace Team A's path up through the bracket — just 3-4 games, not hundreds.
>
...
Try nonce=1: Hash = 8f3a2b1c... ❌ Too big
Try nonce=2: Hash = 2a1b3c4d... ❌ Too big
...
Try nonce=847293847: Hash = 00000000000000000004a3b2... ✅ Less than target!
```
...
> 🔧 **Technical Detail**: The UTXO model is fundamentally different from Ethereum's account model and Solana's account model. Understanding this distinction matters for smart contract design.
...
> 💡 **Key Insight**: Bitcoin Script's limitations are features, not bugs. They make Bitcoin predictable and secure but unsuitable for complex applications. This is why Ethereum was created.
