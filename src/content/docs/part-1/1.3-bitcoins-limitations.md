---
title: "1.3 Bitcoin's Limitations"
description: "Learn about 1.3 Bitcoin's Limitations in the Solana onboarding curriculum."
---
...
- ✅ Send BTC if two of three keys sign (multisig)
- ✅ Lock BTC until a specific time

**What Bitcoin Cannot Do**
- ❌ Automatically pay rent on the 1st of every month
- ❌ Release funds only if a package is delivered
- ❌ Create a voting system where tokens represent shares
- ❌ Run a decentralized exchange
- ❌ Issue loans with programmed interest payments

> 💡 **Key Insight**: Bitcoin is digital *gold*, not digital *law*. It stores and transfers value but can't enforce complex agreements.
...
- Faster blocks → more orphaned blocks (wasted work), less security
- Slower blocks → more security, but worse user experience

Satoshi chose 10 minutes as a balance, optimized for a global network with high latency.

**Why 3-7 TPS?**

Block size is limited to 4M weight units (often ~1-2 MB of block data):
- Average transaction: ~500 bytes
- Blocks every 10 minutes
- Math: ~2 MB / 500 bytes / 600 seconds ≈ 7 TPS
...
> ⚠️ **Common Misconception**: "Bitcoin could just increase block size to scale." This is far more contentious than it sounds — it led to the biggest civil war in Bitcoin's history.

### Limitation 3: It's Expensive (Energy)

Bitcoin mining consumes approximately **120-200 TWh per year** — similar to a medium-sized country.
...
> ⚠️ **Data Disclaimer**: Performance and energy estimates vary by source and
> methodology. Verify current figures from sources like [ccaf.io/cbeci](https://ccaf.io/cbeci)
> and network explorers (as of those sources).

**Why so much?**

Proof of Work requires energy expenditure proportional to network security:
- More value to protect → more incentive for attackers
- More hashpower → higher attack cost → more security
...
> 💡 **Key Insight**: Whether PoW energy use is "justified" depends on your view of Bitcoin's value. But it creates political and environmental pressures that motivate alternatives.
...
> 📊 **Business Context**: The Block Size Wars demonstrate a fundamental tension: Bitcoin's lack of central governance makes it censorship-resistant but also makes upgrades extremely difficult. This is a feature *and* a bug.
...
> 📊 **Business Context**: Lightning demonstrates that base layer limitations can sometimes be worked around. But it adds complexity and isn't suitable for all use cases (especially smart contracts).
...
- `OP_CAT` (concatenate strings) — disabled
...
> 🔧 **Technical Detail**: Solana's innovation will involve similar parallelism thinking, but applied to smart contract execution rather than simple transfers.
...
- To get 51%: 2.5 million ASICs × $5,000 = **$12.5 billion hardware**
- Electricity: 2.5M × 3,500W = 8.75 GW continuous
...
> 💡 **Key Insight**: Bitcoin's security scales with the value it protects. As BTC price increases, attack cost increases. This is "security through economics."
...
> 🔧 **Technical Detail**: This long-term sustainability question is why alternative consensus mechanisms (PoS) became attractive. Ethereum's switch to PoS eliminated this concern for their network.
...
These limitations aren't failures — they're design choices. Bitcoin optimizes for one thing: being secure, censorship-resistant money.
