---
title: "2.1 Ethereum: Code on the Blockchain"
description: "Learn about 2.1 Ethereum: Code on the Blockchain in the Solana onboarding curriculum."
---
...
Ethereum introduced the concept of **smart contracts** — self-executing code that lives on the blockchain. This single innovation created an entirely new category: decentralized applications (dApps), DeFi, NFTs, DAOs, and eventually, the need for faster blockchains like Solana.
...
> ⚠️ **Data Disclaimer**: Some smart contracts are upgradeable or pausable by design.
...
> 💡 **Key Insight**: Smart contracts don't just automate existing processes — they enable entirely new ones that weren't possible with trusted intermediaries.
...
1. A SPECIFICATION (the rules)
   └── Written in the Yellow Paper and EIPs
   └── Defines what valid transactions/blocks look like

2. CLIENT SOFTWARE (the implementation)
   └── Programs that follow the specification
   └── Examples: Geth, Nethermind, Besu, Erigon

3. A NETWORK (the participants)
   └── Thousands of computers running client software
   └── All agreeing on the same state
```
...
Your Computer
     ...
     ▼
┌─────────────────┐
│  Geth Client    │  ← Software following Ethereum rules
├─────────────────┤
│ - Blockchain DB │  ← Full history of all transactions
│ - State DB      │  ← Current balances and contract data
│ - P2P Network   │  ← Connections to other nodes
│ - JSON-RPC API  │  ← How wallets/apps talk to it
└─────────────────┘
     │
     ▼
   Network
...
> 💡 **Key Insight**: The protocol is just rules. Multiple independent teams write software that follows those rules. The network is all the computers running that software, agreeing on state.
...
Users pay: `gas_used × gas_price = fee in ETH`
...
> 📊 **Business Context**: Gas fees are the "cost of decentralization." Centralized systems (AWS) charge for compute too, but with simpler pricing. Blockchain fees fluctuate with demand, creating UX challenges.
...
Ethereum contracts can call other contracts. This creates **composability** — the ability to combine protocols like Lego blocks:
...
Borrow millions, use it, repay — all in one transaction:
...
> 📊 **Business Context**: Composability is Ethereum's moat. Every new protocol adds capabilities that all other protocols can use. This network effect is extremely powerful — and why "Ethereum killers" struggle to gain traction.
...
│  Memory         │ Volatile, byte-addressed, per-call   │
│  Storage        │ Persistent, 256-bit key-value store  │
│  Calldata       │ Input to current call (read-only)    │
│  Return Data    │ Output from last call                │
│  Program Counter│ Current bytecode position            │
│  Gas            │ Remaining computation budget         │
└────────────────────────────────────────────────────────┘
```
...
│ Address → Account                                     │
│                                                       │
│ 0x123... → {nonce, balance, storageRoot, codeHash}   │
│ 0x456... → {nonce, balance, storageRoot, codeHash}   │
└──────────────────────────────────────────────────────┘
                    │
                    ▼ (for contracts)
           ┌─────────────────┐
...
```
User Transaction
  └── Contract A (receives 100,000 gas)
        └── Calls Contract B (forwards 63/64 of remaining gas)
              └── Calls Contract C (forwards 63/64 of remaining)
                    └── ...
```
