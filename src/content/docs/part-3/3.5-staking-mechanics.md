---
title: "3.5 Staking Mechanics"
description: "Learn about 3.5 Staking Mechanics in the Solana onboarding curriculum."
---
...
> ⚠️ **Note**: Numeric examples in this module are illustrative. Verify current parameters and network values in Solana’s staking documentation and on-chain dashboards (as of [docs.solanalabs.com/consensus/stake-delegation-and-rewards](https://docs.solanalabs.com/consensus/stake-delegation-and-rewards)).
...
New SOL Minted:
  ...
  - Defined by the protocol’s inflation schedule
...
Minus validator commission (e.g., 5%):
  Effective APY = nominal APY × (1 - commission)
```
...
```
Delegate → Activating → Active → Deactivating → Withdrawable
          (warmup)                  (cooldown)
```
...
Example (illustrative):
You stake 100 SOL
  → SOL is locked during the cooldown period (can span multiple epochs)
  → Can't use in DeFi
  → Opportunity cost
```
...
│  total_deposited: <sol>                     │
│  total_mSOL: <msol>                         │
│  msol_price: total_deposited / total_mSOL   │
│                                              │
...
2. **Rewards come from inflation** — check current inflation parameters
3. **APY depends on staking rate** — fewer stakers = higher APY
4. **Validator choice matters** — commission, performance, decentralization
5. **Slashing policy varies** — check current enforcement and delegator risks
