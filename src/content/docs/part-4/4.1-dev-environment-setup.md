---
title: "4.1 Development Environment Setup"
description: "Learn about 4.1 Development Environment Setup in the Solana onboarding curriculum."
---
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│   LANGUAGE          FRAMEWORK         TESTING          DEPLOYMENT   │
│   ┌─────────┐      ┌─────────┐       ┌─────────┐      ┌─────────┐  │
│   │  Rust   │ ───► │ Anchor  │ ───►  │ Mocha   │ ───► │ Devnet  │  │
│   └─────────┘      └─────────┘       └─────────┘      └─────────┘  │
│                                                                      │
│   Programs are     Anchor adds       Tests run in     Deploy to     │
│   written in       macros and        TypeScript/JS    test network  │
│   Rust             validation                         first         │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
- **Anchor framework** often reduces development time for common patterns
...
| **Anchor** | Widely used standard in the Solana ecosystem | Native Rust (more complex) |
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│   1. WRITE CODE     2. BUILD        3. TEST         4. DEPLOY       │
│   ┌─────────┐      ┌─────────┐     ┌─────────┐     ┌─────────┐     │
│   │ lib.rs  │ ───► │ anchor  │ ──► │ anchor  │ ──► │ anchor  │     │
│   │         │      │ build   │     │ test    │     │ deploy  │     │
│   └─────────┘      └─────────┘     └─────────┘     └─────────┘     │
│                                                                      │
│   Edit Rust        Compiles to     Runs local      Pushes to        │
│   program code     .so binary      validator       devnet/mainnet   │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### Toolchain Output Map

```
Rust program
  → Anchor build
  → .so binary (on-chain)
  → IDL (client interface)
  → TypeScript types (frontend use)
```

**Build Artifacts:**
- `.so` file — compiled program binary
- `idl.json` — interface definition (like an API spec)
- TypeScript types — auto-generated client types
...
> ⚠️ **Warning**: Never use a development keypair with real funds. Generate separate keys for mainnet.
...
1. **Rust is required** — Solana programs compile to BPF bytecode
2. **Anchor simplifies development** — use it unless you have a reason not to
3. **Devnet is for development** — free SOL, no real value at risk
4. **Local validator is faster** — use for rapid iteration
5. **Keep environments separate** — different keypairs for devnet/mainnet
