---
title: "4.3 Deep Dive: Accounts & PDAs"
description: "Learn about 4.3 Deep Dive: Accounts & PDAs in the Solana onboarding curriculum."
---
...
Every piece of data on Solana lives in an account. Understanding accounts deeply — how they're created, owned, modified, and derived — is essential for building secure, efficient programs.
...
```mermaid
flowchart TD
    subgraph Lockers["Accounts = Storage Lockers"]
        L1["Locker #1<br/>Owner: You<br/>Data: $500"]
        L2["Locker #2<br/>Owner: App<br/>Data: Game state"]
        L3["Locker #3<br/>Owner: Token<br/>Data: 1000 USDC"]
    end

    subgraph Properties["Every locker has:"]
        P1[An address - how to find it]
        P2[An owner - who controls it]
        P3[Data - what's stored]
        P4[Balance - rent payment]
    end

    style Lockers stroke:#FFD700,stroke-width:3px
```
...
- Regular wallets are like lockers with keys — you need the key (private key) to open them
- PDAs are like the vending machine's internal cash box — only the machine itself can access it
- The machine (program) doesn't need a physical key — it proves it's the machine using math

**Why use PDAs?**
- Programs can control accounts without needing private keys
- Users can find PDA addresses deterministically (predictably)
- Safe from human error — no keys to lose

### PDA Derivation (Visual)

```mermaid
flowchart TD
    A[Seeds + Program ID + Bump] --> B[find_program_address]
    B --> C[PDA Address]
...
```mermaid
flowchart TD
    subgraph Owner["ONLY THE OWNER CAN:"]
        O1[Modify the account's data]
        O2[Debit lamports from the account]
    end

    subgraph Anyone["ANYONE CAN:"]
        A1[Read the account's data]
        A2[Credit lamports to the account]
    end

    subgraph Special["SPECIAL CASES:"]
        S1[System Program owns wallet accounts]
        S2[BPF Loader owns program accounts]
        S3[Your program owns accounts it creates]
    end

    style Owner stroke:#FFD700,stroke-width:3px,color:#FFD700
```
...
flowchart LR
    subgraph Problem["PROBLEM"]
        P1[Regular accounts need<br/>a private key to sign]
        P2[Programs don't have<br/>private keys]
        P3[How can a program<br/>control an account?]
    end

    subgraph Solution["SOLUTION"]
        S1[PDAs derived from<br/>seeds + program ID]
        S2[They're off the curve<br/>no valid private key]
        S3[Program can sign for them<br/>using invoke_signed]
    end
...
1. **Accounts are owned by programs** — only owners can modify data
2. **PDAs are deterministic** — same seeds always give same address
3. **Store the bump** — saves computation on subsequent calls
4. **Use Anchor constraints** — they prevent common vulnerabilities
5. **Check ownership and signatures** — critical for security
6. **Rent exemption is standard** — always pay for permanent accounts
