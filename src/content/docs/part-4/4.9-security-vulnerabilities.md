---
title: "4.9 Security & Common Vulnerabilities"
description: "Learn about 4.9 Security & Common Vulnerabilities in the Solana onboarding curriculum."
---
...
TRADITIONAL SOFTWARE:
  Bug found → Patch deployed → Users update
  Recovery possible, data can be restored

BLOCKCHAIN PROGRAMS:
  Bug found → Funds already stolen
...
│      "Is this account owned by the right program?"                  │
│                                                                      │
│   2. MISSING SIGNER CHECKS                                          │
│      "Did the right person authorize this?"                         │
│                                                                      │
│   3. ARITHMETIC OVERFLOW/UNDERFLOW                                  │
│      "Will this math break with large numbers?"                     │
│                                                                      │
│   4. TYPE CONFUSION                                                 │
│      "Is this account the type I expect?"                           │
│                                                                      │
│   5. PDA SUBSTITUTION                                               │
│      "Can someone pass a fake PDA?"                                 │
│                                                                      │
│   6. REINITIALIZATION                                               │
│      "Can an account be set up twice?"                              │
│                                                                      │
│   7. CLOSING ACCOUNT VULNERABILITIES                                │
│      "Can rent extraction be exploited?"                            │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
Program expects a "vault" account
  → Attacker supplies a fake account
  → Program transfers funds to attacker
```
...
✓ All program code
✓ Client code that builds transactions
...
1. **Assume adversarial inputs** — validate everything
2. **Owner checks are critical** — use Account<T> or verify manually
3. **Signer checks are critical** — require signatures for actions
4. **Use checked math** — overflow/underflow are real attacks
5. **Validate PDAs** — always verify seeds
6. **Prevent reinitialization** — use init or check flag
7. **Close accounts properly** — zero data, transfer rent
8. **Verify CPI targets** — don't trust any program
9. **Defense in depth** — multiple validation layers
10. **Get audited** — human review is essential
