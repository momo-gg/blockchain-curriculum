---
title: "5.3 Transaction Building"
description: "Learn about 5.3 Transaction Building in the Solana onboarding curriculum."
---

> *"A well-constructed transaction is like a well-written sentence — every part serves a purpose."*
...
│   └─────────────────────────────────────────────────────────────┘   │
│                                                                      │
│   Max size: ~1,232 bytes (legacy; v0 differs with ALTs)              │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
> ⚠️ **Trading Risk Vocabulary (Applied to Transactions)**
...
```
Built → Signed → Submitted → Processed → Confirmed → Finalized
```
...
1. **Structure matters** — header, signatures, keys, instructions
2. **Order instructions correctly** — compute budget first
3. **Use versioned transactions** — for more accounts via ALTs
4. **Simulate first** — catch errors before sending
5. **Handle blockhash expiration** — retry with fresh hash
6. **Partial signing** — for multi-sig scenarios
7. **Durable nonces** — for offline signing
8. **Batch when possible** — parallel independent transactions
