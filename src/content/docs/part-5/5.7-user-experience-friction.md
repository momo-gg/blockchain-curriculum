---
title: "5.7 User Experience & Transaction Friction"
description: "Learn about 5.7 User Experience & Transaction Friction in the Solana onboarding curriculum."
---
...
```
Install wallet → Fund wallet → First transaction → First failure → Trust lost?
```
...
1. **Explain the cause** — "You need SOL for fees" beats "Transaction failed."
2. **Show next actions** — Provide a clear fix (fund, retry, adjust slippage).
3. **Separate user error vs network error** — Ownership changes behavior.
4. **Confirm intent** — Summarize what the user is signing in plain language.
...
### Error Messaging Ladder

```
What failed? → Why it failed? → What to do next?
```

If any step is missing, users lose trust fast.
...
- **Fee/rent errors** — insufficient SOL, account not rent-exempt
- **Blockhash errors** — blockhash not found, transaction expired
- **Instruction errors** — custom program error codes, constraint failures
- **RPC errors** — timeouts, transport errors, rate limits
...
```
created → signed → submitted → processed → confirmed → finalized
```
...
1. **Most UX pain is error ambiguity** — explain, do not just report
2. **Fees and rent are the top surprise** — surface them early
3. **Pending is not processing** — communicate finality clearly
4. **Simulations are UX tools** — use them for clarity
5. **Trust grows from predictability** — consistent status states matter
