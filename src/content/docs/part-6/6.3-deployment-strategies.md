---
title: "6.3 Deployment Strategies"
description: "Learn about 6.3 Deployment Strategies in the Solana onboarding curriculum."
---
...
Deploying Solana programs requires careful planning. On-chain programs can be immutable or upgradeable depending on the loader and authority configuration; deployment costs real money, and mistakes can be costly. Understanding deployment strategies, upgrade patterns, and environment management is essential.
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    Solana Deployment Environments                    │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
...
│   │              │         │              │        │              │ │
│   │ • Free       │   →     │ • Free SOL   │   →    │ • Real SOL   │ │
│   │ • Fast       │         │ • Public     │        │ • Real users │ │
│   │ • Reset any  │         │ • May reset  │        │ • Permanent  │ │
│   │   time       │         │ • Rate limit │        │              │ │
│   └──────────────┘         └──────────────┘        └──────────────┘ │
│                                                                      │
│   TESTNET (Limited use - mostly for validator testing)              │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
```
Build → Deploy → Verify → Upgrade (optional)
  │        │        │         │
  ▼        ▼        ▼         ▼
...
### Upgrade Authority (Mental Model)

```
Authority key controls upgrades
  → Multisig reduces single-key risk
  → Revoke authority to make immutable
```
...
| | Test coverage targets met | Engineering |
...
| Large programs | Varies by rent parameters |

*Note: Program size limits are protocol-defined and may change; verify current limits in Solana docs (as of [docs.solanalabs.com](https://docs.solanalabs.com)).*
...
### Sealevel Runtime Details (Deployment Impact)

Sealevel is Solana's parallel runtime. Deployment and operations should account for:

**Account Locks**
- Each transaction declares writable/read-only accounts.
- Two transactions cannot write the same account in the same slot.
- Hot accounts (program state, global config) can become bottlenecks.

**Compute Units (CU)**
- Each instruction consumes CU; default budget is limited.
- Complex instructions may need a higher CU limit + priority fee.
- Over-budget transactions fail even if logic is correct.

**Parallelism Strategy**
- Split state across multiple accounts to reduce lock contention.
- Avoid single global writable accounts in high-traffic paths.
- Use CPI sparingly; each CPI adds overhead and CU usage.
...
async function verifyDeployment(
  ...
  if (deployedHash === expectedHash) {
    console.log("✓ Binary hash matches");
  } else {
    console.log("✗ Binary hash mismatch");
...
1. **Test extensively on devnet** — simulate mainnet conditions
2. **Use multisig for mainnet** — never single-key authority
3. **Verify deployments** — check binary hashes match
4. **Automate with CI/CD** — reduce human error
5. **Plan for upgrades** — use upgradeable programs initially
6. **Document everything** — program IDs, authorities, versions
7. **Have rollback plans** — buffer previous versions
8. **Monitor after deploy** — catch issues early
