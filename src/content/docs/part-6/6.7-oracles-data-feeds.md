---
title: "6.7 Oracles & Data Feeds"
description: "Learn about 6.7 Oracles & Data Feeds in the Solana onboarding curriculum."
---
...
### Staleness Check (Mental Model)

```
Read oracle price
  → If update too old, reject action
  → If fresh, proceed
```
...
> ⚠️ **Data Disclaimer**: Oracle providers and feed details change frequently.
...
1. **Data source integrity** — Where does the data come from?
2. **Update cadence** — How frequently is data refreshed?
3. **Incident history** — How have outages been handled in the past?
4. **Cost model** — Subscription vs per-request pricing
5. **Coverage** — Do you need niche assets or only majors?
...
- [ ] Why can’t Solana programs read external APIs directly?
...
1. **Oracles are critical infrastructure** — treat them like production dependencies
2. **Staleness and manipulation are the main risks** — design guardrails
3. **Fallback behavior must be defined** — do not improvise in incidents
4. **Monitoring is mandatory** — detect feed issues before users do
5. **Validate feeds on-chain** — never trust unverified inputs
